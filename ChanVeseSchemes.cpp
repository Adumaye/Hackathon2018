...
double ChanVeseSchemes::Correction(const std::vector<std::vector<double>>& phi)
{
// Calcul de C1 et C2
double dom_plus=0., dom_moins=0, z_plus=0., z_moins=0., C1, C2;
	...
	for (int i=0; i<phi.size() ; i++)
	{
		for (int j=0; j<phi[0].size() ; j++)
		{
			...
			z_plus += max (_u0_v[i][j]*phi[i][j]/max(abs(phi[i][j]),1.E-16),0.);
			z_moins -= max (-_u0_v[i][j]*phi[i][j]/max(abs(phi[i][j]),1.E-16),0.);
